...
/// <summary>
/// Regex-based database population that uses the REAL database (not in-memory)
/// </summary>
class RegexRealDatabasePopulation
{
    private const string UserSecretsId = "7d5de198-3095-4d2d-acda-c2631c63e9b6";

    public static async Task RunRegexRealDatabasePopulation(string[] args)
    {
        AnsiConsole.Write(
            new Panel(new Text("Regex REAL Database Population", style: "bold"))
                .BorderColor(Color.Red)
                .Header("[red]PRODUCTION DATABASE - BE CAREFUL![/]")
                .Padding(1, 0));

        AnsiConsole.MarkupLine("[red]??  WARNING: This will modify the REAL database![/]");
        AnsiConsole.MarkupLine("[green]Using regex pattern matching - no AI required![/]");
        AnsiConsole.WriteLine();

        if (!AnsiConsole.Confirm("[yellow]Are you sure you want to populate the REAL database?[/]"))
        {
        ...
        try
        {
            ...
            // Add Entity Framework with REAL PostgreSQL database
            var connectionString = configuration.GetConnectionString("DefaultConnection");
            if (string.IsNullOrEmpty(connectionString))
            {
                AnsiConsole.MarkupLine("[red]? No connection string found! Check your configuration.[/]");
                return;
            }
            ...
            var logger = serviceProvider.GetRequiredService<ILogger<RegexRealDatabasePopulation>>();

            AnsiConsole.MarkupLine("[green]? Connected to real database[/]");

            // Test database connection
            try
            {
                await dbContext.Database.CanConnectAsync();
                AnsiConsole.MarkupLine("[green]? Database connection verified[/]");
            }
            catch (Exception ex)
            {
            ...
            // Show what we found before saving
            AnsiConsole.Write(new Rule("[blue]Preview of Data to Save[/]"));
            AnsiConsole.MarkupLine("[dim]First 10 water bodies to be saved:[/]");
            foreach (var wb in waterBodies.Take(10))
            {
                AnsiConsole.MarkupLine($"[dim]  • {wb.Name} ({wb.WaterType}) - {wb.County} County[/]");
            }
            if (waterBodies.Count > 10)
            {
                AnsiConsole.MarkupLine($"[dim]  ... and {waterBodies.Count - 10} more[/]");
            }
            AnsiConsole.WriteLine();

            if (!AnsiConsole.Confirm($"[yellow]Proceed to save {waterBodies.Count} water bodies to the real database?[/]"))
            {
                AnsiConsole.MarkupLine("[dim]Operation cancelled.[/]");
                return;
            }
            ...
            AnsiConsole.MarkupLine($"[cyan]Current database counts - States: {existingStatesCount}, Counties: {existingCountiesCount}, Water Bodies: {existingWaterBodiesCount}[/]");
            ...
            if (minnesotaState == null)
            {
                ...
                logger.LogInformation("Created Minnesota state record");
            ...
            // Create counties with simple progress
            var createdCounties = 0;
            var countyMap = new Dictionary<string, County>();

            AnsiConsole.MarkupLine($"[yellow]Creating counties... (0/{counties.Count})[/]");
            ...
            for (int i = 0; i < counties.Count; i++)
            {
                ...
                try
                {
                    ...
                    // Update progress every 5 counties
                    if ((i + 1) % 5 == 0 || i == counties.Count - 1)
                    {
                        AnsiConsole.MarkupLine($"[dim]Counties progress: {i + 1}/{counties.Count} (Created: {createdCounties})[/]");
                    }
                }
                catch (Exception ex)
                {
            ...
            // Create water bodies with simple progress
            var createdWaterBodies = 0;
            var skippedWaterBodies = 0;

            AnsiConsole.MarkupLine($"[yellow]Creating water bodies... (0/{waterBodies.Count})[/]");

            for (int i = 0; i < waterBodies.Count; i++)
            {
                var waterBodyInfo = waterBodies[i];
                ...
                try
                {
                    ...
                    if (existingWaterBody == null)
                    {
                        ...
                        if (createdWaterBodies % 25 == 0)
                        {
                            await dbContext.SaveChangesAsync();
                            logger.LogDebug($"Saved batch: {createdWaterBodies} water bodies so far");
                        }
                    }
                    else
                    {
                    ...
                    // Update progress every 25 water bodies
                    if ((i + 1) % 25 == 0 || i == waterBodies.Count - 1)
                    {
                        AnsiConsole.MarkupLine($"[dim]Water bodies progress: {i + 1}/{waterBodies.Count} (Created: {createdWaterBodies}, Skipped: {skippedWaterBodies})[/]");
                    }
                }
                catch (Exception ex)
                {
            ...
            AnsiConsole.MarkupLine($"[yellow]??  Skipped {skippedWaterBodies} existing or failed water bodies[/]");
            ...
            summaryTable.AddRow("Water Bodies Skipped", skippedWaterBodies.ToString());
            ...
            summaryTable.AddRow("Database", "[red]REAL PRODUCTION DATABASE[/]");

            AnsiConsole.Write(summaryTable);

            // Show sample data from REAL database
            AnsiConsole.Write(new Rule("[blue]Sample Data from REAL Database[/]"));
            ...
            try
            {
                ...
                {
                    AnsiConsole.MarkupLine("[green]Latest water bodies in the real database:[/]");
                    foreach (var sample in sampleWaterBodies)
                    {
                        AnsiConsole.MarkupLine($"• [blue]{sample.Name}[/] ({sample.WaterType}) - [yellow]{sample.County?.Name}[/] County [dim](Created: {sample.CreatedAt:yyyy-MM-dd HH:mm})[/]");
                    }
                }
                else
                {
                    AnsiConsole.MarkupLine("[red]? No water bodies found in real database![/]");
                }
            }
            catch (Exception ex)
            {
            ...
            AnsiConsole.MarkupLine($"\n[green]?? Regex REAL database population completed![/]");
            AnsiConsole.MarkupLine("[red]??  Data has been saved to the PRODUCTION database.[/]");
        }
        catch (Exception ex)
        {
    ...
    // Use the same extraction methods as the in-memory version
    ...
    // Copy all the helper methods from RegexDatabasePopulation

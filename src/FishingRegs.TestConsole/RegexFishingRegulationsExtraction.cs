...
class RegexFishingRegulationsExtraction
{
            ...
            try
            {
                ...
                var existingRegulation = await dbContext.FishingRegulations
                    .FirstOrDefaultAsync(fr => 
                        ...
                        fr.DailyLimit == regInfo.DailyLimit &&
                        fr.PossessionLimit == regInfo.PossessionLimit);
                ...
                if (existingRegulation == null)
                {
                    ...
                    {
                        // Log and continue - this regulation already exists
                        logger.LogDebug($"Duplicate regulation skipped for {regInfo.WaterBodyName} - {regInfo.SpeciesName}");
                    }
                }
            }
            catch (Exception ex)
    ...
    public static List<FishingRegulationInfo> ExtractFishingRegulations(string text)
    {
        ...
        return regulations.DistinctBy(r => $"{r.WaterBodyName}|{r.County}|{r.SpeciesName}|{r.DailyLimit}|{r.PossessionLimit}").ToList();
    }
    ...
    private static List<FishingRegulationInfo> ParseRegulationText(string regulationText, string waterBodyName, string county)
    {
        ...
        // Extract daily limits using various patterns
        var dailyLimitPatterns = new[]
        {
            @"daily\s*limit\s*(?:for\s*)?([a-z\s]+?)\s*(?:to\s*|of\s*|is\s*)(\d+)",
            @"reduces?\s*(?:the\s*)?daily\s*limit\s*(?:for\s*)?([a-z\s]+?)\s*(?:to\s*|of\s*)(\d+)",
            @"([a-z\s]+?)\s*daily\s*limit\s*(?:of\s*|is\s*)?(\d+)"
        };

        foreach (var pattern in dailyLimitPatterns)
        {
            var matches = Regex.Matches(regulationText, pattern, RegexOptions.IgnoreCase);
            
            foreach (Match match in matches)
            {
                var speciesText = match.Groups[1].Value.Trim();
                var limitText = match.Groups[2].Value.Trim();
        ...
        // Extract possession limits
        var possessionMatches = Regex.Matches(regulationText, @"possession\s*limit\s*(?:is\s*)?(\d+)", RegexOptions.IgnoreCase);
        foreach (Match possessionMatch in possessionMatches)
        {
            if (int.TryParse(possessionMatch.Groups[1].Value, out var possessionLimit))
            {
                var species = ExtractSpeciesFromContext(regulationText);
        ...
        // If no specific regulations found, try to extract any species mentioned
        if (regulations.Count == 0)
        {
            var allSpecies = ExtractAllSpeciesFromContext(regulationText);
    ...
    public static string MapSpeciesName(string speciesText)
    {
        ...
        return normalized switch
        {
            ...
            var s when s.Contains("stream trout") => "Brook Trout", // Fix for Stream Trout issue
            ...
            var s when s.Contains("northern") => "Northern Pike",
    ...
    public static string ExtractSpeciesFromContext(string text)
    {
        var speciesVariations = new Dictionary<string, string[]>
        {
            ...
            ["Brook Trout"] = new[] { "brook trout", "brookies", "brook", "stream trout" },
...
/// <summary>
/// Information about a water body extracted from text
/// </summary>
